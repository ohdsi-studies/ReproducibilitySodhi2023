...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (4057518,195847,4113427,4198430,4010028)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (195847)
...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (4046080)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (4046080)
  and c.invalid_reason is null
...
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= DATEADD(day,0,P.START_DATE) AND A.END_DATE >= DATEADD(day,0,P.END_DATE) AND A.END_DATE <= P.OP_END_DATE ) cc 
...
WHERE V.visit_concept_id in (9202,5084,8756,8947)
-- End Condition Occurrence Criteria
